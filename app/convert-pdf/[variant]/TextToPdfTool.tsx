...
// Letter page, 12pt Helvetica, 40pt margins, 18pt line height
const PAGE_W = 612;
const PAGE_H = 792;
const MARGIN = 40;
const FONT_SIZE = 12;
...
// Wrap a single line at word boundaries, hard-breaking words longer than MAX_CHARS
...
export default function TextToPdfTool({ variant }: Props) {
  ...
  async function handleConvert() {
    ...
    try {
      ...
      // Expand all lines with word-wrapping
  ...
  function handleDownload() {
    ...
    const baseName = file?.name.replace(/\.txt$/i, "") ?? "document";
  ...
  return (
    <>
      <input ref={inputRef} type="file" accept=".txt" style={{ display: "none" }} onChange={handleFileChange} />
