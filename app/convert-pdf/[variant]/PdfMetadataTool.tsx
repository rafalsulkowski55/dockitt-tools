...
export default function PdfMetadataTool({ variant }: Props) {
  ...
  function handleFileSelect(f: File) {
    ...
    setFile(f); setStatus("idle"); setErrorMessage(""); setDownloadUrl(null); setMetadata(null);
  ...
  function handleReset() {
    setFile(null); setStatus("idle"); setErrorMessage(""); setDownloadUrl(null); setMetadata(null);
  ...
  async function handleExtract() {
    ...
    setStatus("processing"); setErrorMessage(""); setDownloadUrl(null); setMetadata(null);
  ...
  function handleDownload() {
    ...
    const a = document.createElement("a"); a.href = downloadUrl; a.download = `${baseName}-metadata.json`; a.click();
  }

  const isProcessing = status === "processing";
  const isReady = !!file && !isProcessing;

  const metaRows: { label: string; value: string | number | null }[] = metadata
    ? [
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <button disabled={!isReady} onClick={handleExtract}
        ...
        {status === "done" && metadata && downloadUrl && (
          <div style={{ border: "1px solid #bfdbfe", borderRadius: "12px", overflow: "hidden" }}>
            <div style={{ background: "#eff6ff", padding: "12px 16px" }}>
              <p style={{ fontSize: "15px", fontWeight: 700, color: "#2563eb", margin: "0 0 2px" }}>✅ Metadata extracted</p>
              <p style={{ fontSize: "12px", color: "#16a34a", margin: 0 }}>Your JSON file is ready to download</p>
            </div>
            ...
            <div style={{ padding: "12px 16px", background: "#fff", borderBottom: "1px solid #e5e7eb" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                {metaRows.map(({ label, value }) => (
                  <div key={label} style={{ display: "flex", gap: "8px", fontSize: "12px", alignItems: "baseline" }}>
                    <span style={{ color: "#6b7280", minWidth: "90px", flexShrink: 0 }}>{label}</span>
                    <span style={{ color: value !== null ? "#111" : "#d1d5db", fontStyle: value !== null ? "normal" : "italic", wordBreak: "break-word" }}>
            ...
            <div style={{ padding: "10px 16px", background: "#f9fafb", borderTop: "1px solid #e5e7eb" }}>
              <button onClick={handleReset} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, fontSize: "12px", color: "#2563eb", fontWeight: 500 }}>→ Inspect another file</button>
            </div>
          </div>
        )}
