...
function truncate(text: string, maxChars: number): string {
  const s = String(text ?? "");
  if (s.length <= maxChars) return s.padEnd(maxChars, " ");
  return s.slice(0, maxChars - 1) + "…";
}

export default function CsvToPdfTool({ variant }: Props) {
  ...
  async function handleConvert() {
    ...
    const interval = setInterval(() => {
      setProgress((p) => (p < 40 ? p + Math.random() * 10 : p));
    }, 200);
    ...
    try {
      ...
      const allCols = result.meta.fields ?? [];
      const cols = allCols.slice(0, MAX_COLS);
      const rows = result.data as Record<string, string>[];

      if (cols.length === 0) throw new Error("No columns found in the CSV file.");

      const usableWidth = PAGE_W - 2 * MARGIN;
      const colWidth = usableWidth / cols.length;
      // Approximate chars per column: Helvetica ~6.5pt wide per char at 11pt
      const charsPerCol = Math.max(4, Math.floor(colWidth / 6.5));
      ...
      // Header row
      const headerText = cols.map((h) => truncate(h, charsPerCol)).join(" ");
      page.drawText(headerText, { x: MARGIN, y: y - HEADER_SIZE, size: HEADER_SIZE, font: fontBold, color: rgb(0, 0, 0) });
      y -= HEADER_LINE_H;

      // Separator line (drawn as repeated dashes)
      const sepLen = cols.length * (charsPerCol + 1);
      page.drawText("-".repeat(Math.min(sepLen, 100)), { x: MARGIN, y: y - ROW_SIZE, size: ROW_SIZE, font: fontRegular, color: rgb(0.6, 0.6, 0.6) });
      y -= ROW_LINE_H;

      const total = rows.length;
      for (let i = 0; i < total; i++) {
        if (y - ROW_SIZE < MARGIN) {
          page = pdfDoc.addPage([PAGE_W, PAGE_H]);
          y = PAGE_H - MARGIN;
        }
        const row = rows[i];
        const rowText = cols.map((col) => truncate(String(row[col] ?? ""), charsPerCol)).join(" ");
        page.drawText(rowText, { x: MARGIN, y: y - ROW_SIZE, size: ROW_SIZE, font: fontRegular, color: rgb(0, 0, 0) });
        y -= ROW_LINE_H;
        setProgress(40 + Math.round((i / total) * 55));
      }
  ...
  function handleDownload() {
    ...
    const baseName = file?.name.replace(/\.csv$/i, "") ?? "document";
        ...
        <div onDrop={handleDrop} onDragOver={(e) => e.preventDefault()}
          ...
          {file ? (
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              <div style={{ width: "40px", height: "52px", borderRadius: "6px", background: "#eff6ff", border: "1px solid #bfdbfe", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: "10px", fontWeight: 700, color: "#2563eb", textTransform: "uppercase" }}>CSV</div>
            ...
            <div style={{ textAlign: "center", padding: "8px 0" }}>
              <div style={{ fontSize: "13px", color: "#666", marginBottom: "10px" }}>Drag & drop your CSV file here or</div>
        ...
        {isProcessing && (
          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", color: "#6b7280" }}>
              <span>Building table...</span><span>{Math.round(progress)}%</span>
            </div>
