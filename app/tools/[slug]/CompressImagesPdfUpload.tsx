...
const TOOL_NAME = "compress-images-pdf";
...
const QUALITY_OPTIONS: { id: Quality; label: string; desc: string; dpi: number }[] = [
  { id: "low", label: "Low", desc: "Smallest file — 72 DPI", dpi: 72 },
  { id: "medium", label: "Medium", desc: "Balanced — 120 DPI", dpi: 120 },
  { id: "high", label: "High", desc: "Best quality — 150 DPI", dpi: 150 },
];

export default function CompressImagesPdfUpload() {
  ...
  const [resultInfo, setResultInfo] = useState<{ originalSize: number; compressedSize: number; reduction: number } | null>(null);
  ...
  function handleFileSelect(f: File) {
    setErrorMessage(""); setDownloadUrl(null); setProgress(0); setResultInfo(null);
  ...
  function handleReset() {
    ...
    setDownloadUrl(null); setProgress(0); setErrorMessage(""); setResultInfo(null);
    if (inputRef.current) inputRef.current.value = "";
  }

  async function handleProcess() {
    if (!file) return;
    const dpi = QUALITY_OPTIONS.find(q => q.id === quality)?.dpi ?? 120;
    setStatus("validating"); setErrorMessage(""); setDownloadUrl(null); setProgress(0); setResultInfo(null);
    ...
    try {
      ...
      const completeRes = await fetch("/api/uploads/complete", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ storageKey, toolSlug: TOOL_NAME, processingParams: { quality: dpi } }) });
      if (!completeRes.ok) { const err = await completeRes.json(); throw new Error(err.error ?? "Processing failed"); }
      setProgress(100);
      const result = await completeRes.json();
      setPendingDownload({ storageKey, filename: `compressed-images-${file.name}`, toolSlug: TOOL_NAME, toolPath: `/tools/${TOOL_NAME}`, timestamp: Date.now() });
      setResultInfo({ originalSize: result.originalSize, compressedSize: result.compressedSize, reduction: result.reduction });
  ...
  async function handleDownload() {
    ...
    const a = document.createElement("a"); a.href = downloadUrl; a.download = `compressed-images-${file?.name ?? "file.pdf"}`; a.click();
  }

  const isProcessing = ["validating", "uploading", "processing"].includes(status);
  const processingLabel = status === "uploading" ? "Uploading to secure storage..." : status === "processing" ? "Compressing images..." : "Validating...";
    ...
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          <p style={{ fontSize: "13px", fontWeight: 500, color: "#374151", margin: 0 }}>Image quality:</p>
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            {QUALITY_OPTIONS.map((q) => (
              <button key={q.id} onClick={() => setQuality(q.id)}
                style={{ padding: "8px 16px", borderRadius: "8px", border: "1px solid", borderColor: quality === q.id ? "#2563eb" : "#e5e7eb", background: quality === q.id ? "#2563eb" : "#fff", cursor: "pointer", textAlign: "left" }}>
                <p style={{ margin: 0, fontSize: "13px", fontWeight: 600, color: quality === q.id ? "#fff" : "#111" }}>{q.label}</p>
                <p style={{ margin: 0, fontSize: "11px", color: quality === q.id ? "#bfdbfe" : "#9ca3af" }}>{q.desc}</p>
              </button>
            ))}
          </div>
        </div>
        ...
        {status === "done" && downloadUrl && resultInfo && (
          <div style={{ border: "1px solid #bfdbfe", borderRadius: "12px", overflow: "hidden" }}>
            <div style={{ background: "#eff6ff", padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px", flexWrap: "wrap" }}>
              <div>
                <p style={{ fontSize: "16px", fontWeight: 700, color: "#2563eb", margin: "0 0 2px" }}>
                  {resultInfo.reduction > 0 ? `${resultInfo.reduction}% smaller` : "Images compressed"}
                </p>
                <p style={{ fontSize: "12px", color: "#16a34a", margin: 0 }}>✓ Ready to download</p>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", flexShrink: 0 }}>
                <div style={{ textAlign: "center" }}>
                  <p style={{ fontSize: "10px", color: "#9ca3af", margin: "0 0 2px", textTransform: "uppercase" }}>Before</p>
                  <p style={{ fontSize: "13px", fontWeight: 600, color: "#374151", margin: 0 }}>{formatSize(resultInfo.originalSize)}</p>
                </div>
                <span style={{ color: "#9ca3af" }}>→</span>
                <div style={{ textAlign: "center" }}>
                  <p style={{ fontSize: "10px", color: "#16a34a", margin: "0 0 2px", textTransform: "uppercase" }}>After</p>
                  <p style={{ fontSize: "13px", fontWeight: 600, color: "#15803d", margin: 0 }}>{formatSize(resultInfo.compressedSize)}</p>
                </div>
              </div>
            </div>
            ...
            <div style={{ padding: "10px 16px", background: "#f9fafb", borderTop: "1px solid #e5e7eb" }}>
              <button onClick={handleReset} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, fontSize: "12px", color: "#2563eb", fontWeight: 500 }}>→ Compress another PDF</button>
            </div>
          </div>
        )}
      </div>
