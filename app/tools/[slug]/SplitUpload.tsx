...
export default function SplitUpload() {
  ...
  function handleReset() {
    setFile(null); setStatus("idle"); setDownloadUrl(null); setErrorMessage("");
    setTotalPages(null); setFromPage("1"); setToPage("1"); setProgress(0);
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
        ...
        {file && (
          <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
            <span style={{ fontSize: "13px", color: "#374151", fontWeight: 500 }}>Pages from</span>
            <input type="number" min="1" value={fromPage} onChange={(e) => setFromPage(e.target.value)}
              style={{ width: "64px", padding: "7px 10px", border: "1px solid #d1d5db", borderRadius: "8px", fontSize: "14px", outline: "none" }} />
            <span style={{ fontSize: "13px", color: "#374151" }}>to</span>
            <input type="number" min="1" value={toPage} onChange={(e) => setToPage(e.target.value)}
              style={{ width: "64px", padding: "7px 10px", border: "1px solid #d1d5db", borderRadius: "8px", fontSize: "14px", outline: "none" }} />
            {totalPages && <span style={{ fontSize: "12px", color: "#9ca3af" }}>(PDF has {totalPages} pages)</span>}
          </div>
        )}
        ...
        {status === "processing" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", color: "#6b7280" }}>
              <span>Splitting PDF...</span><span>{progress}%</span>
            </div>
        ...
        {status === "done" && downloadUrl && (
          <div style={{ border: "1px solid #bfdbfe", borderRadius: "12px", overflow: "hidden" }}>
            <div style={{ background: "#eff6ff", padding: "12px 16px" }}>
              <p style={{ fontSize: "15px", fontWeight: 700, color: "#2563eb", margin: "0 0 2px" }}>✅ Split complete</p>
              <p style={{ fontSize: "12px", color: "#16a34a", margin: 0 }}>Pages {fromPage}–{toPage} extracted successfully</p>
            </div>
            ...
            <div style={{ padding: "10px 16px", background: "#f9fafb", borderTop: "1px solid #e5e7eb" }}>
              <button onClick={handleReset} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, fontSize: "12px", color: "#2563eb", fontWeight: 500 }}>→ Split another PDF</button>
            </div>
          </div>
        )}
