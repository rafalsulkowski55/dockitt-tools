...
const TOOL_NAME = "remove-metadata";
...
export default function RemoveMetadataUpload() {
  ...
  async function handleProcess() {
    ...
    try {
      ...
      setPendingDownload({ storageKey, filename: `clean-${file.name}`, toolSlug: TOOL_NAME, toolPath: `/tools/${TOOL_NAME}`, timestamp: Date.now() });
  ...
  async function handleDownload() {
    ...
    const a = document.createElement("a"); a.href = downloadUrl; a.download = `clean-${file?.name ?? "file.pdf"}`; a.click();
  }

  const isProcessing = ["validating", "uploading", "processing"].includes(status);
  const processingLabel = status === "uploading" ? "Uploading to secure storage..." : status === "processing" ? "Removing metadata..." : "Validating...";
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
        ...
        <div style={{ background: "#f8faff", border: "1px solid #bfdbfe", borderRadius: "8px", padding: "10px 12px" }}>
          <p style={{ fontSize: "12px", color: "#4b5563", margin: 0, lineHeight: 1.5 }}>
            ℹ️ Removes: Author, Title, Subject, Keywords, Creator, Producer, Creation Date, Modification Date.
          </p>
        </div>
        ...
        {status === "done" && downloadUrl && (
          <div style={{ border: "1px solid #bfdbfe", borderRadius: "12px", overflow: "hidden" }}>
            <div style={{ background: "#eff6ff", padding: "12px 16px" }}>
              <p style={{ fontSize: "15px", fontWeight: 700, color: "#2563eb", margin: "0 0 2px" }}>✅ Metadata removed</p>
              <p style={{ fontSize: "12px", color: "#16a34a", margin: 0 }}>Your PDF contains no identifying metadata</p>
            </div>
            ...
            <div style={{ padding: "10px 16px", background: "#f9fafb", borderTop: "1px solid #e5e7eb" }}>
              <button onClick={handleReset} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, fontSize: "12px", color: "#2563eb", fontWeight: 500 }}>→ Clean another PDF</button>
            </div>
          </div>
        )}
      </div>
