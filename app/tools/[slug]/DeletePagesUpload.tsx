...
export default function DeletePagesUpload() {
  ...
  async function handleProcess() {
    ...
    try {
      ...
      const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `deleted-pages-${file.name}`; a.click();
        ...
        {file && (
          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            <label style={{ fontSize: "13px", fontWeight: 500, color: "#374151" }}>
              Pages to delete{totalPages ? ` (PDF has ${totalPages} pages)` : ""}:
            </label>
            <input
              type="text"
              value={pages}
        ...
        {status === "done" && (
          <div style={{ border: "1px solid #bfdbfe", borderRadius: "12px", overflow: "hidden" }}>
            <div style={{ background: "#eff6ff", padding: "12px 16px" }}>
              <p style={{ fontSize: "15px", fontWeight: 700, color: "#2563eb", margin: "0 0 2px" }}>✅ Pages deleted successfully</p>
            ...
            <div style={{ padding: "10px 16px", background: "#f9fafb", borderTop: "1px solid #e5e7eb" }}>
              <button onClick={handleReset} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, fontSize: "12px", color: "#2563eb", fontWeight: 500 }}>→ Delete pages from another PDF</button>
            </div>
          </div>
        )}
