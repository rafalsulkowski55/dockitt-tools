...
export default function DeletePagesUpload() {
  ...
  async function handleProcess() {
    ...
    try {
      ...
      const a = document.createElement("a"); a.href = url; a.download = `deleted-pages-${file.name}`; a.click();
    ...
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
        <div onDrop={handleDrop} onDragOver={(e) => e.preventDefault()} onClick={() => inputRef.current?.click()} style={{ border: "2px dashed #bfdbfe", borderRadius: "12px", padding: "24px", textAlign: "center", background: "#f8faff", cursor: "pointer" }}>
          ...
          <label htmlFor="delete-pages-upload" style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", padding: "10px 20px", background: "#2563eb", color: "#ffffff", borderRadius: "10px", fontWeight: 600, cursor: "pointer", fontSize: "14px" }} onClick={(e) => e.stopPropagation()}>Choose PDF</label>
        ...
        {status === "done" && <div style={{ padding: "14px 16px", background: "#eff6ff", color: "#1d4ed8", border: "1px solid #bfdbfe", borderRadius: "10px", fontSize: "14px" }}>✅ Done. The selected pages have been removed.</div>}
      </div>
    </>
  );
}