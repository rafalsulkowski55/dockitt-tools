...
export default function PdfToTextUpload() {
  ...
  async function handleProcess() {
    ...
    try {
      ...
      setPendingDownload({ storageKey, filename: `text-${file.name.replace(".pdf", ".txt")}`, toolSlug: TOOL_NAME, toolPath: `/tools/${TOOL_NAME}`, timestamp: Date.now() });
  ...
  async function handleDownload() {
    ...
    const a = document.createElement("a"); a.href = downloadUrl; a.download = `text-${file?.name?.replace(".pdf", ".txt") ?? "output.txt"}`; a.click();
  }

  const isProcessing = ["validating", "uploading", "processing"].includes(status);
  const processingLabel = status === "uploading" ? "Uploading to secure storage..." : status === "processing" ? "Extracting text..." : "Validating...";
        ...
        {status === "done" && downloadUrl && (
          <div style={{ border: "1px solid #bfdbfe", borderRadius: "12px", overflow: "hidden" }}>
            <div style={{ background: "#eff6ff", padding: "12px 16px" }}>
              <p style={{ fontSize: "15px", fontWeight: 700, color: "#2563eb", margin: "0 0 2px" }}>✅ Text extracted successfully</p>
              <p style={{ fontSize: "12px", color: "#16a34a", margin: 0 }}>Your .txt file is ready to download</p>
            </div>
