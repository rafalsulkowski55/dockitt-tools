...
export default function OcrUpload() {
  ...
  async function handleDownload() {
    ...
    const a = document.createElement("a"); a.href = downloadUrl; a.download = `ocr-${file?.name ?? "file.pdf"}`; a.click();
  }

  const isProcessing = ["validating", "uploading", "processing"].includes(status);
  const processingLabel =
    status === "uploading" ? "Uploading to secure storage..." :
    status === "processing" ? "Running OCR — this may take up to 60 seconds..." :
    "Validating...";
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
        ...
        <div style={{ background: "#f8faff", border: "1px solid #bfdbfe", borderRadius: "8px", padding: "10px 12px" }}>
          <p style={{ fontSize: "12px", color: "#4b5563", margin: 0, lineHeight: 1.5 }}>
            ℹ️ OCR makes scanned PDFs searchable and selectable. Processing may take 30–60 seconds.
          </p>
        </div>

        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <button disabled={!file || isProcessing} onClick={handleOcr}
        ...
        {status === "done" && downloadUrl && (
          <div style={{ border: "1px solid #bfdbfe", borderRadius: "12px", overflow: "hidden" }}>
            <div style={{ background: "#eff6ff", padding: "12px 16px" }}>
              <p style={{ fontSize: "15px", fontWeight: 700, color: "#2563eb", margin: "0 0 2px" }}>✅ OCR complete</p>
              <p style={{ fontSize: "12px", color: "#16a34a", margin: 0 }}>Your PDF is now searchable and selectable</p>
            </div>
            ...
            <div style={{ padding: "10px 16px", background: "#f9fafb", borderTop: "1px solid #e5e7eb" }}>
              <button onClick={handleReset} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, fontSize: "12px", color: "#2563eb", fontWeight: 500 }}>→ Run OCR on another PDF</button>
            </div>
          </div>
        )}
