...
export default function OcrUpload() {
  ...
  async function handleOcr() {
    ...
    try {
      ...
      const completeRes = await fetch("/api/uploads/complete", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ storageKey, toolSlug: TOOL_NAME, processingParams: {} }) });
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
        ...
        <button disabled={!file || isProcessing} onClick={handleOcr} style={{ padding: "12px 24px", background: file && !isProcessing ? "#2563eb" : "#d1d5db", color: "#fff", border: "none", borderRadius: "10px", fontWeight: 600, fontSize: "15px", cursor: file && !isProcessing ? "pointer" : "not-allowed", width: "fit-content" }}>
          {status === "uploading" ? "Uploading..." : status === "processing" ? "Running OCR..." : status === "validating" ? "Validating..." : "Run OCR"}
        </button>

        {isProcessing && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", color: "#666", marginBottom: "6px" }}>
              <span>{status === "uploading" ? "Uploading to secure storage..." : status === "processing" ? "Running OCR — this may take up to 60 seconds..." : "Validating..."}</span>
              <span>{progress}%</span>
            </div>
        ...
        {status === "done" && downloadUrl && (
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            <div style={{ padding: "20px", background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: "10px" }}>
              <p style={{ fontSize: "18px", fontWeight: 700, color: "#2563eb", margin: "0 0 6px" }}>✅ OCR complete</p>
              <p style={{ fontSize: "13px", color: "#4b5563", margin: 0 }}>Your PDF is now searchable and selectable.</p>
            </div>
            <button onClick={handleDownload} style={{ padding: "14px 28px", background: "#16a34a", color: "#fff", border: "none", borderRadius: "10px", fontWeight: 600, fontSize: "15px", cursor: "pointer", width: "fit-content" }}>⬇ Download searchable PDF</button>
            <button onClick={handleReset} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, fontSize: "13px", color: "#2563eb", fontWeight: 500, textAlign: "left" }}>→ Run OCR on another PDF</button>
          </div>
        )}
      </div>
    </>
