...
export default function DarkModePdfUpload() {
  ...
  async function handleProcess() {
    ...
    try {
      ...
      setPendingDownload({ storageKey, filename: `dark-mode-${file.name}`, toolSlug: TOOL_NAME, toolPath: `/tools/${TOOL_NAME}`, timestamp: Date.now() });
  ...
  async function handleDownload() {
    ...
    const a = document.createElement("a"); a.href = downloadUrl; a.download = `dark-mode-${file?.name ?? "file.pdf"}`; a.click();
  }

  const isProcessing = ["validating", "uploading", "processing"].includes(status);
  const processingLabel = status === "uploading" ? "Uploading to secure storage..." : status === "processing" ? "Inverting colours..." : "Validating...";
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
        ...
        <div style={{ background: "#f8faff", border: "1px solid #bfdbfe", borderRadius: "8px", padding: "10px 12px" }}>
          <p style={{ fontSize: "12px", color: "#4b5563", margin: 0, lineHeight: 1.5 }}>
            🌙 Inverts all colours in the PDF. Run the tool again on the result to restore the original colours.
          </p>
        </div>
        ...
        {status === "done" && downloadUrl && (
          <div style={{ border: "1px solid #bfdbfe", borderRadius: "12px", overflow: "hidden" }}>
            <div style={{ background: "#1e1b4b", padding: "12px 16px" }}>
              <p style={{ fontSize: "15px", fontWeight: 700, color: "#e0e7ff", margin: "0 0 2px" }}>🌙 Dark mode PDF ready</p>
              <p style={{ fontSize: "12px", color: "#a5b4fc", margin: 0 }}>Colours have been inverted</p>
            </div>
            ...
            <div style={{ padding: "10px 16px", background: "#f9fafb", borderTop: "1px solid #e5e7eb" }}>
              <button onClick={handleReset} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, fontSize: "12px", color: "#2563eb", fontWeight: 500 }}>→ Convert another PDF</button>
            </div>
          </div>
        )}
      </div>
