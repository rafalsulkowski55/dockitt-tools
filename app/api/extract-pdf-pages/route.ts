...
export async function POST(req: NextRequest) {
  try {
    ...
    const pagesToExtract = (formData.get("pages") as string ?? "").split(",").map(s => parseInt(s.trim())).filter(n => !isNaN(n));
    ...
    const invalidPages = pagesToExtract.filter(p => p < 1 || p > totalPages);
