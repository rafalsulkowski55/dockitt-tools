import { NextRequest, NextResponse } from "next/server";
import { PDFDocument } from "pdf-lib";

export const runtime = "nodejs";
export const maxDuration = 30;

export async function POST(req: NextRequest) {
  try {
    const formData = await req.formData();
    const file = formData.get("file") as File | null;
    const pagesToDelete = (formData.get("pages") as string ?? "").split(",").map(s => parseInt(s.trim())).filter(n => !isNaN(n));

    if (!file) {
      return NextResponse.json({ error: "No file provided" }, { status: 400 });
    }

    if (pagesToDelete.length === 0) {
      return NextResponse.json({ error: "No pages specified" }, { status: 400 });
    }

    const arrayBuffer = await file.arrayBuffer();
    const pdfDoc = await PDFDocument.load(arrayBuffer, { ignoreEncryption: true });
    const totalPages = pdfDoc.getPageCount();

    const invalidPages = pagesToDelete.filter(p => p < 1 || p > totalPages);
    if (invalidPages.length > 0) {
      return NextResponse.json(
        { error: `Invalid page numbers: ${invalidPages.join(", ")}. PDF has ${totalPages} pages.` },
        { status: 400 }
      );
    }

    if (pagesToDelete.length >= totalPages) {
      return NextResponse.json(
        { error: "Cannot delete all pages from a PDF." },
        { status: 400 }
      );
    }

    const sortedDesc = [...pagesToDelete].sort((a, b) => b - a);
    sortedDesc.forEach(pageNum => {
      pdfDoc.removePage(pageNum - 1);
    });

    const newBytes = await pdfDoc.save({ useObjectStreams: true });

    return new NextResponse(Buffer.from(newBytes), {
      status: 200,
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="deleted-pages.pdf"`,
        "X-Total-Pages": totalPages.toString(),
      },
    });
  } catch (err) {
    console.error("Delete PDF pages error:", err);
    return NextResponse.json({ error: "Failed to process PDF" }, { status: 500 });
  }
}