...
import { getAllDevTools, getDevTool } from "@/data/dev-tools/tools";
import type { DevTool } from "@/data/dev-tools/tools";
import type { TextTool } from "@/data/text-tools/tools";
import type { FileTool } from "@/data/file-tools/tools";
import Link from "next/link";
import JsonFormatterTool from "@/app/text-tools/components/JsonFormatterTool";
import Base64EncoderTool from "@/app/file-tools/components/Base64EncoderTool";
import UrlEncoderTool from "@/app/file-tools/components/UrlEncoderTool";
import PasswordGeneratorTool from "@/app/file-tools/components/PasswordGeneratorTool";
import UuidGeneratorTool from "@/app/file-tools/components/UuidGeneratorTool";
import ColorPickerTool from "@/app/file-tools/components/ColorPickerTool";
import CssMinifierTool from "@/app/file-tools/components/CssMinifierTool";

function DevToolRouter({ tool }: { tool: DevTool }) {
  switch (tool.slug) {
    case "json-formatter":     return <JsonFormatterTool     tool={tool as unknown as TextTool} />;
    case "base64-encoder":     return <Base64EncoderTool     tool={tool as unknown as FileTool} />;
    case "url-encoder":        return <UrlEncoderTool        tool={tool as unknown as FileTool} />;
    case "password-generator": return <PasswordGeneratorTool tool={tool as unknown as FileTool} />;
    case "uuid-generator":     return <UuidGeneratorTool     tool={tool as unknown as FileTool} />;
    case "color-picker":       return <ColorPickerTool       tool={tool as unknown as FileTool} />;
    case "css-minifier":       return <CssMinifierTool       tool={tool as unknown as FileTool} />;
    default:                   return null;
  }
}
...
export async function generateStaticParams() {
  return getAllDevTools().map((t) => ({ slug: t.slug }));
}

export default async function DevToolSlugPage({ params }: PageProps) {
  const { slug } = await params;
  const tool = getDevTool(slug);
  const all = getAllDevTools();
  if (!tool) notFound();
            ...
            <div style={{ alignSelf: "start" }}>
              <nav style={{ marginBottom: "16px" }}>
                <ol style={{ display: "flex", alignItems: "center", gap: "8px", listStyle: "none", margin: 0, padding: 0, fontSize: "13px", color: "#9ca3af" }}>
                  ...
                  <li><Link href="/dev-tools" style={{ color: "#9ca3af", textDecoration: "none" }}>Developer Tools</Link></li>
                  <li style={{ color: "#d1d5db" }}>›</li>
            ...
            <div style={{ alignSelf: "start", background: "#ffffff", border: "1px solid #e5e7eb", borderRadius: "16px", overflow: "hidden" }}>
              ...
              <div style={{ padding: "12px 16px", borderBottom: "1px solid #e5e7eb", background: "#f9fafb" }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "5px" }}>
                  {all.map((item) => (
                    <Link key={item.slug} href={`/dev-tools/${item.slug}`}
                      style={{ padding: "5px 10px", borderRadius: "7px", fontSize: "11px", fontWeight: 500, textDecoration: "none", background: item.slug === slug ? "#7c3aed" : "#fff", color: item.slug === slug ? "#fff" : "#374151", border: "1px solid", borderColor: item.slug === slug ? "#7c3aed" : "#e5e7eb" }}>
                      {item.name}
                    </Link>
                  ))}
                </div>
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: "10px", background: "#f0fdf4", borderBottom: "1px solid #bbf7d0", padding: "10px 16px", fontSize: "13px", color: "#14532d" }}>
                ...
                <span><strong>Runs entirely in your browser.</strong> Your data never leaves your device — no server, complete privacy.</span>
              </div>

              <div style={{ padding: "24px" }}>
                <DevToolRouter tool={tool} />
              </div>
            </div>
      ...
      <section style={{ background: "#f9fafb" }}>
        <div style={{ maxWidth: "780px", margin: "0 auto", padding: "40px 24px" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "32px" }}>
            ...
            <section style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: "16px", padding: "24px" }}>
              <h2 style={{ marginTop: 0, fontSize: "20px", borderLeft: "3px solid #7c3aed", paddingLeft: "12px" }}>How to use</h2>
            ...
            <section style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: "16px", padding: "24px" }}>
              <h2 style={{ marginTop: 0, fontSize: "20px", borderLeft: "3px solid #7c3aed", paddingLeft: "12px" }}>FAQ</h2>
