...
export default function RemoveExifTool({ tool }: { tool: EditImageTool }) {
  ...
  function handleDownload() {
    ...
    const a = document.createElement("a"); a.href = downloadUrl; a.download = `${baseName}-clean.${ext}`; a.click();
  }
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <button disabled={!isReady} onClick={handleProcess}
            ...
            {isProcessing ? "Removing EXIF..." : "Remove EXIF Data"}
          </button>
        ...
        {status === "done" && downloadUrl && (
          <div style={{ border: "1px solid #bfdbfe", borderRadius: "12px", overflow: "hidden" }}>
            <div style={{ background: "#eff6ff", padding: "12px 16px" }}>
              <p style={{ fontSize: "15px", fontWeight: 700, color: "#2563eb", margin: "0 0 2px" }}>✅ EXIF data removed</p>
              <p style={{ fontSize: "12px", color: "#16a34a", margin: 0 }}>All metadata has been stripped from the image</p>
            </div>
            ...
            <div style={{ padding: "10px 16px", background: "#f9fafb", borderTop: "1px solid #e5e7eb" }}>
              <button onClick={handleReset} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, fontSize: "12px", color: "#2563eb", fontWeight: 500 }}>→ Clean another image</button>
            </div>
          </div>
        )}
