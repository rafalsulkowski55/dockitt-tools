...
export default function GrayscaleImageTool({ tool }: { tool: EditImageTool }) {
  ...
  function handleDownload() {
    ...
    const a = document.createElement("a"); a.href = downloadUrl; a.download = `${baseName}-grayscale.${ext}`; a.click();
  }
        ...
        {status === "done" && downloadUrl && (
          <div style={{ border: "1px solid #bfdbfe", borderRadius: "12px", overflow: "hidden" }}>
            <div style={{ background: "#eff6ff", padding: "12px 16px" }}>
              ...
              <p style={{ fontSize: "12px", color: "#16a34a", margin: 0 }}>Your grayscale image is ready</p>
            </div>
            ...
            <div style={{ padding: "10px 16px", background: "#f9fafb", borderTop: "1px solid #e5e7eb" }}>
              <button onClick={handleReset} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, fontSize: "12px", color: "#2563eb", fontWeight: 500 }}>→ Convert another image</button>
            </div>
          </div>
        )}
