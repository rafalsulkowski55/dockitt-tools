...
export default function FlipImageTool({ tool }: { tool: EditImageTool }) {
  ...
  function handleReset() {
    setFile(null); setStatus("idle"); setErrorMessage(""); setDownloadUrl(null); setDirection("horizontal");
  ...
  async function handleProcess() {
    ...
    try {
      ...
      if (direction === "horizontal") {
        ctx.translate(img.naturalWidth, 0); ctx.scale(-1, 1);
      } else {
        ctx.translate(0, img.naturalHeight); ctx.scale(1, -1);
      }
      ...
      setErrorMessage("Flip failed. Please try a different image.");
      setStatus("error");
    }
  }
  ...
  function handleDownload() {
    ...
    const a = document.createElement("a"); a.href = downloadUrl; a.download = `${baseName}-flipped.${ext}`; a.click();
  }
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
        ...
        {file && (
          <div style={{ display: "flex", gap: "8px" }}>
            <button onClick={() => setDirection("horizontal")} style={btnStyle(direction === "horizontal")}>Flip Horizontal</button>
            <button onClick={() => setDirection("vertical")} style={btnStyle(direction === "vertical")}>Flip Vertical</button>
          </div>
        )}
        ...
        {status === "done" && downloadUrl && (
          <div style={{ border: "1px solid #bfdbfe", borderRadius: "12px", overflow: "hidden" }}>
            <div style={{ background: "#eff6ff", padding: "12px 16px" }}>
              <p style={{ fontSize: "15px", fontWeight: 700, color: "#2563eb", margin: "0 0 2px" }}>✅ Flip complete</p>
              <p style={{ fontSize: "12px", color: "#16a34a", margin: 0 }}>Your flipped image is ready</p>
            </div>
            ...
            <div style={{ padding: "10px 16px", background: "#f9fafb", borderTop: "1px solid #e5e7eb" }}>
              <button onClick={handleReset} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, fontSize: "12px", color: "#2563eb", fontWeight: 500 }}>→ Flip another image</button>
            </div>
          </div>
        )}
