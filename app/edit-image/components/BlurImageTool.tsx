...
export default function BlurImageTool({ tool }: { tool: EditImageTool }) {
  ...
  function handleReset() {
    setFile(null); setStatus("idle"); setErrorMessage(""); setDownloadUrl(null); setBlurPx(5);
  ...
  async function handleProcess() {
    ...
    try {
      ...
      ctx.drawImage(img, -blurPx * 2, -blurPx * 2, canvas.width + blurPx * 4, canvas.height + blurPx * 4);
      ctx.filter = "none";
  ...
  function handleDownload() {
    ...
    const a = document.createElement("a"); a.href = downloadUrl; a.download = `${baseName}-blurred.${ext}`; a.click();
  }
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
        ...
        {file && (
          <div>
            <label style={{ fontSize: "12px", color: "#6b7280", display: "block", marginBottom: "4px" }}>Blur intensity: {blurPx}px</label>
            <input type="range" min="1" max="20" value={blurPx} onChange={(e) => setBlurPx(Number(e.target.value))}
              style={{ width: "100%" }} />
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "11px", color: "#9ca3af", marginTop: "2px" }}>
              <span>1px (light)</span><span>20px (heavy)</span>
            </div>
          </div>
        )}
        ...
        {status === "done" && downloadUrl && (
          <div style={{ border: "1px solid #bfdbfe", borderRadius: "12px", overflow: "hidden" }}>
            <div style={{ background: "#eff6ff", padding: "12px 16px" }}>
              <p style={{ fontSize: "15px", fontWeight: 700, color: "#2563eb", margin: "0 0 2px" }}>✅ Blur applied</p>
              <p style={{ fontSize: "12px", color: "#16a34a", margin: 0 }}>Your blurred image is ready</p>
            </div>
            ...
            <div style={{ padding: "10px 16px", background: "#f9fafb", borderTop: "1px solid #e5e7eb" }}>
              <button onClick={handleReset} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, fontSize: "12px", color: "#2563eb", fontWeight: 500 }}>→ Blur another image</button>
            </div>
          </div>
        )}
