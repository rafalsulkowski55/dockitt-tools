...
export const convertVariants: ConvertVariant[] = [
  ...
  },
  {
    slug: "pdf-to-text",
    name: "PDF to Text",
    title: "Convert PDF to Text Online",
    description: "Extract all text from a PDF file and download it as a plain .txt file.",
    ...
    faqs: [
      {
        question: "What content is included in the extracted text?",
        answer: "All readable text in the PDF is extracted including body text, headings, captions, and table content. Images are not included in the output. If text appears in an image rather than as actual PDF text data, it will not be extracted — use the OCR PDF tool first to add a text layer to image-based PDFs.",
      },
      {
        question: "Will the formatting and layout be preserved?",
        answer: "No. The output is plain text without any formatting. Bold, italic, font sizes, colours, and layout positioning are not preserved. The text is extracted in reading order as the PDF renderer encounters it, which may differ from the visual order for complex multi-column layouts.",
      },
      {
        question: "Does this work on scanned PDFs?",
        answer: "No. A scanned PDF contains images of text rather than actual text data. Extracting text from a scanned PDF will produce an empty or near-empty file. Use the Dockitt OCR PDF tool first to add a searchable text layer to the scanned document, then use this tool to extract the text.",
      },
      {
        question: "The extracted text has unusual characters or garbled words.",
        answer: "This happens when the PDF uses embedded or custom fonts that do not map cleanly to standard Unicode characters. Some PDFs use font encoding tricks that make the text visually correct but prevent it from being extracted accurately. Unfortunately there is no reliable fix for this — it is a limitation of how that PDF was created.",
      },
      {
        question: "Can I extract text from a password-protected PDF?",
        answer: "No. The PDF must be unlocked before the text can be extracted. Use the Dockitt Unlock PDF tool to remove the password, then extract the text from the unlocked version.",
      },
      {
        question: "Is my file safe when I upload it?",
        answer: "Yes. Your file is sent over an encrypted connection and processed on a secure server. It is deleted immediately after the text file is returned to you. Dockitt does not store, read, or share your files.",
      },
    ],
  },
  {
    ...
    accept: ".pdf",
    howTo: [
      "Click 'Choose PDF' and select the PDF you want to convert to PowerPoint.",
      "Click 'Convert to PPTX' to start the conversion on our secure server.",
      "Download the .pptx file and open it in PowerPoint, Google Slides, or LibreOffice Impress.",
    ],
    faqs: [
      {
        question: "Will the slides look exactly like the PDF pages?",
        answer: "For simple text-based presentations the conversion is usually close to the original. For PDFs with complex graphic layouts, custom fonts, or heavy use of positioned elements, some differences in layout are expected. The text content should be present and editable even when the visual layout differs.",
      },
      {
        question: "Can I edit the text in the converted PowerPoint?",
        answer: "Yes. The text from the PDF is placed into editable text boxes in the PowerPoint file. You can click on any text element and edit it freely in your presentation software.",
      },
      {
        question: "Images from the PDF are missing in the PowerPoint file.",
        answer: "LibreOffice extracts images during conversion but some PDFs embed images in formats that do not transfer cleanly. If critical images are missing, you can export them using the PDF to JPG or PDF to PNG tool and insert them manually into the correct slides.",
      },
      {
        question: "The fonts look different in the PowerPoint file.",
        answer: "If the PDF uses fonts that are not available on the conversion server, LibreOffice substitutes them with standard alternatives. The text content is preserved but the visual appearance may differ slightly from the original. For presentations where fonts are critical, converting on your own machine using software that has access to your installed fonts will produce better results.",
      },
      {
        question: "Can I convert a scanned PDF to PowerPoint?",
        answer: "Not directly. A scanned PDF contains images of text rather than actual text data. Run the Dockitt OCR PDF tool on the scanned document first to add a text layer, then convert it to PowerPoint.",
      },
      {
        question: "Is my file safe when I upload it?",
        answer: "Yes. Your file is sent over an encrypted connection and processed on a secure server. It is deleted immediately after the converted file is returned to you. Dockitt does not store, read, or share your files.",
      },
    ],
  },
  {
    ...
    accept: ".pdf",
    howTo: [
      "Click 'Choose PDF' and select the PDF containing the data you want to convert.",
      "Click 'Convert to XLSX' to start the conversion on our secure server.",
      "Download the .xlsx file and open it in Excel, Google Sheets, or LibreOffice Calc.",
    ],
    faqs: [
      {
        question: "Will the tables in my PDF be extracted correctly?",
        answer: "Simple, clearly structured tables with visible borders and consistent row and column alignment convert well. Complex tables with merged cells, nested tables, or unusual layouts may need manual cleanup after conversion. The accuracy of the extraction depends heavily on how the table was structured in the original PDF.",
      },
      {
        question: "The data appears in one column instead of multiple columns.",
        answer: "This can happen when the PDF table does not use visible grid lines and the column structure is implied by spacing rather than explicit separators. LibreOffice may interpret the rows as single text entries in this case. You may need to use the data splitting features in Excel or Google Sheets to separate the values into columns.",
      },
      {
        question: "Numbers appear as text in the Excel file.",
        answer: "PDF text extraction treats all content as text strings. Numbers and dates from a PDF may come through as formatted text rather than numeric values. In Excel you can select these cells and use the Convert to Number option, or use the VALUE function to convert them to proper numeric format.",
      },
      {
        question: "Can I extract data from a scanned PDF?",
        answer: "No. Scanned PDFs contain images rather than actual text and table data. Use the Dockitt OCR PDF tool first to add a text layer to the scanned document, then convert it to Excel. OCR accuracy on tables varies, so some manual correction may still be needed.",
      },
      {
        question: "Is my file safe when I upload it?",
        answer: "Yes. Your file is sent over an encrypted connection and processed on a secure server. It is deleted immediately after the converted file is returned to you. Dockitt does not store, read, or share your files.",
      },
      {
        ...
        answer: "Files up to 10MB are supported. For larger files, consider splitting the PDF into sections using the Split PDF tool, converting each section separately, and then combining the results in your spreadsheet application.",
      },
    ],
  },
  {
    ...
    description: "Convert PowerPoint presentations (.pptx) to PDF files that look great everywhere.",
    longDescription: "A PowerPoint file looks different on every computer. Fonts substitute, animations are lost, and layouts shift depending on the version of PowerPoint, the operating system, and the fonts installed on the recipient's machine. Converting your presentation to PDF before sharing it eliminates all of these variables. A PDF looks identical on every device and in every PDF viewer. Your carefully arranged slides, your fonts, your images, and your layout are all preserved exactly as you created them. This tool converts .pptx and .ppt files to PDF on our secure server using LibreOffice. Your file is deleted immediately after processing.",
    primaryKeyword: "powerpoint to pdf",
    inputFormat: "pptx",
    outputFormat: "pdf",
    inputLabel: "Choose PPTX",
    accept: ".pptx,.ppt",
    howTo: [
      "Click 'Choose PPTX' and select your PowerPoint file.",
      "Click 'Convert to PDF' to process the file on our secure server.",
      "Download the PDF and verify that the slides look as expected before sharing.",
    ],
    faqs: [
      {
        question: "Will animations and transitions be preserved in the PDF?",
        answer: "No. PDF is a static format and does not support animations or slide transitions. Each slide in the PowerPoint becomes a single static page in the PDF. If your presentation relies heavily on animations to reveal content step by step, each slide will show the final state of all animated elements.",
      },
      {
        question: "Will fonts be embedded correctly in the PDF?",
        answer: "If the fonts used in your presentation are installed on the conversion server, they will be embedded correctly in the PDF. Standard fonts like Arial, Times New Roman, and Calibri convert cleanly. Custom or licensed fonts that are not available on the server will be substituted with similar alternatives, which can affect the visual appearance of text-heavy slides.",
      },
      {
        question: "The PDF has a different layout than my PowerPoint file.",
        answer: "Layout differences usually result from font substitution or from using features that LibreOffice handles differently from Microsoft PowerPoint. For presentations with precise layout requirements, converting directly from PowerPoint using File, Save As, PDF format on your own machine will produce the most accurate result since your fonts and settings are available locally.",
      },
      {
        question: "Can I convert both .pptx and .ppt files?",
        answer: "Yes. Both the older .ppt format and the newer .pptx format are accepted. LibreOffice handles both and the conversion process is identical for either format.",
      },
      {
        question: "Is my file safe when I upload it?",
        answer: "Yes. Your file is sent over an encrypted connection and processed on a secure server. It is deleted immediately after the PDF is returned to you. Dockitt does not store, read, or share your files.",
      },
      {
        question: "What is the maximum file size I can convert?",
        answer: "Files up to 10MB are supported. Large presentations with many high-resolution images may exceed this limit. In that case, try compressing the images in PowerPoint before converting, or split the presentation into sections.",
      },
    ],
  },
  {
    ...
    description: "Convert Excel spreadsheets (.xlsx) to PDF files that display consistently everywhere.",
    longDescription: "Spreadsheets are powerful for working with data but not ideal for sharing. An Excel file looks different depending on the recipient's version of Excel, their screen size, their print settings, and the fonts they have installed. Column widths shift, rows break across pages differently, and formulas are visible instead of just their values. Converting your spreadsheet to PDF locks the layout into place. What you see is what they get, on every device and in every PDF viewer. This tool converts .xlsx and .xls files to PDF on our secure server using LibreOffice. Your file is deleted immediately after processing.",
    primaryKeyword: "excel to pdf",
    inputFormat: "xlsx",
    outputFormat: "pdf",
    inputLabel: "Choose XLSX",
    accept: ".xlsx,.xls",
    howTo: [
      "Click 'Choose XLSX' and select your Excel spreadsheet.",
      "Click 'Convert to PDF' to process the file on our secure server.",
      "Download the PDF and verify the layout and data look correct before sharing.",
    ],
    faqs: [
      {
        question: "Will formulas be shown in the PDF or just their results?",
        answer: "Only the calculated results of formulas are shown in the PDF, not the formulas themselves. The PDF captures the current values in the cells exactly as they appear in the spreadsheet.",
      },
      {
        question: "How are multiple sheets handled in the PDF?",
        answer: "Each sheet in the Excel workbook becomes a section of the PDF. All visible sheets are included in the output. Hidden sheets are not included.",
      },
      {
        question: "The PDF cuts off columns that are visible in Excel.",
        answer: "This happens when the spreadsheet is wider than the page size used for conversion. LibreOffice uses the print area and page setup settings from the spreadsheet. If columns are being cut off, try setting the print area in Excel to include all necessary columns, or use the Fit to Page option in Page Setup before converting.",
      },
      {
        question: "Charts and graphs from Excel appear incorrectly in the PDF.",
        answer: "Most standard charts convert correctly. Complex or custom chart types may not render identically due to differences between how Excel and LibreOffice handle chart rendering. For presentations where chart accuracy is critical, converting directly from Excel using File, Save As, PDF on your own machine will give the most accurate result.",
      },
      {
        question: "Can I convert both .xlsx and .xls files?",
        answer: "Yes. Both the older .xls format and the newer .xlsx format are accepted. LibreOffice handles both and the conversion process is identical for either.",
      },
      {
        question: "Is my file safe when I upload it?",
        answer: "Yes. Your file is sent over an encrypted connection and processed on a secure server. It is deleted immediately after the PDF is returned to you. Dockitt does not store, read, or share your files.",
  ...
  },
  {
    ...
    accept: ".pdf",
    howTo: [
      "Click 'Choose PDF' and select the PDF you want to convert to HTML.",
      "Click 'Convert to HTML' to start the conversion on our secure server.",
      "Download the .html file and open it in a browser or edit it in an HTML editor.",
    ],
    faqs: [
      {
        question: "Will the HTML look exactly like the PDF?",
        answer: "For simple text-based documents the HTML will be close to the original. For PDFs with complex layouts, multiple columns, or precise positioning of elements, the HTML version will be a linearised version of the content — all the text will be present but the visual layout will differ significantly from the PDF.",
      },
      {
        question: "Will images from the PDF be included in the HTML?",
        answer: "LibreOffice attempts to extract images during conversion. Depending on how the images are embedded in the PDF, they may appear as linked files alongside the HTML or may not transfer cleanly. For complex documents with many images, you may need to handle images separately.",
      },
      {
        question: "Can I use the HTML output on my website?",
        answer: "Yes. The HTML file can be opened in any browser and the content can be pasted into a CMS, embedded in a web page, or further edited with an HTML editor. You may want to clean up the markup and add your own styling before publishing.",
      },
      {
        question: "Does this work on scanned PDFs?",
        answer: "No. Scanned PDFs contain images rather than text data. The HTML output from a scanned PDF will contain image references but no readable text. Use the Dockitt OCR PDF tool first to add a text layer, then convert to HTML.",
      },
      {
        question: "Is my file safe when I upload it?",
        answer: "Yes. Your file is sent over an encrypted connection and processed on a secure server. It is deleted immediately after the HTML file is returned to you. Dockitt does not store, read, or share your files.",
      },
      {
        question: "What is the maximum file size I can convert?",
        answer: "Files up to 10MB are supported. Larger PDFs should be split into sections using the Split PDF tool before converting to HTML.",
      },
    ],
  },
];
